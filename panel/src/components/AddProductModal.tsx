...
const AddProductModal: React.FC<AddProductModalProps> = ({ onClose }) => {
  ...
  const [onerilen, setOnerilen] = useState('Hayır');
  const { addItem } = useFirestore<Product>('urunler');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await addItem({ name, price, category, description, image, onerilen, detaylar: '' });
    onClose();
  };
